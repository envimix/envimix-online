...
public static class EnvimaniaEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var normalizedPageSize = Math.Clamp(pageSize ?? 10, 10, 100);
        var totalPages = Math.Max(1, (int)Math.Ceiling(matchingSessionCount / (double)normalizedPageSize));
        ...
        CancellationToken cancellationToken)
    {
        var session = await db.EnvimaniaSessions
            ...
            .Select(x => new
            {
                ...
                x.ServerModeName,
                x.TitleId,
                TitleDisplayName = x.Title == null ? null : x.Title.DisplayName,
                MapUid = x.Map.Id,
                MapName = x.Map.Name,
                MapLaps = x.Map.Laps,
                x.StartedAt,
