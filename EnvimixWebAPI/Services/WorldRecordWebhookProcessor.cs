    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await foreach (var webhook in webhookChannel.Reader.ReadAllAsync(stoppingToken))
        {
            try
            {
                ...
                {
                    fields.Add(new EmbedFieldBuilder().WithName("Previous WR age").WithValue($"{(int)(webhook.NewRecord.DrivenAt - webhook.PrevRecord.DrivenAt).TotalDays} days").WithIsInline(true));
